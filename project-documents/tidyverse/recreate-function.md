## Core project: R Programming | Recreate an existing function

**Primary core objectives targeted: 3, 4, 5**

Recommended additional objectives: 1, 2, 6-11
...
3. Write a “conceptual definition” of the function using comments. Focus on outlining the way the function works for your typical use cases. In other words, if your function has 20 optional arguments but you have only ever used the 2 required arguments, forget about those 20 optional arguments for now; pretend the function works only in the ways you’re already familiar with.
4. Use `function()` to assign the beginnings of a real definition (i.e., assigned code) to your recreation. **Important: Give your function a unique name so you can easily run each at any point.** If you recreate `filter()` , call it something like `my_filter()` , `filter_new()` , or `filter2()` , etc.
