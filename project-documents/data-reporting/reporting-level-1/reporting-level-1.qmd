---
title: "Descriptive statistics & visualization exercise (Structured)"
---


# Objective

The objective of this assignment is to practice using R to calculate and interpret descriptive statistics including simple visualization. Unlike in previous assignments, you will not use a built-in dataset. Instead, you will use data from a published, open-access dataset capturing relationships between math anxiety and self-perception:
...
Modify the violin plot to make it more interpretable. In addition to changing visuals and text, consider whether you might want to add or drop any elements. (Hint: not all likert scales are made equal.)
...
Optional Task 2: These distributions have visualized the raw data. Create a visualization that uses the summarized data to send an interpretable message. Modify the visuals and text as much as you like to create a readable figure.
...
<!-- Create (at least) 2 chunks below to complete the above tasks. Be sure to label informatively and add table/figure caption options when appropriate.-->
...
In your answer below, you can use Quarto markdown references to the tables and figures you created to show more advanced data reporting skills. You can refer to Quarto documentation for examples of how to do this.
...
In D2M-R I we will only lightly touch on running and visualizing correlation and regression analyses. Continue working through the rest of this notebook to show independent learning beyond what we cover this quarter. 

*Note:* Typically core objective points are capped at 4 for highly structured projects. In this case, continuation through the remaining structured (but relatively unfamiliar) tasks may earn 5 or 6 points depending on complexity and initiative.
