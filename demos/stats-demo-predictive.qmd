...
knitr::opts_chunk$set(
	...
	message = FALSE,
	warning = FALSE,
    # comment arg. is the string of text displayed before each line of output
    # knitr sets this to "##" by default, Quarto overwrites to ""
    # I've set it to ">" for illustration, like what you see in actual console output
    # but usually "" is good
	comment = ">" 
  )
```
...
For example, a 1-sample $t$-test on the `mass` variable should return significant results, rejecting the null hypothesis that the true mean is 0. Since mass of observed entities is necessarily a positive value, it is impossible that the true mean would be 0, so we should anticipate a strong and significant effect. 
...
With a $t$-value of `r round(t.test(sw.desc$mass)$statistic, 3)` and a $p$-value of `r round(t.test(sw.desc$mass)$p.value, 3)`, we can pretty confidently reject the null hypothesis that the true mean mass of all the physical beings in the Star Wars universe is 0. The tiny, tiny $p$-value tells us the likelihood that the population's true mean (population = the full group that these observations are just a sample of) is equal to 0. The default $t$-test is 2-tailed, meaning we are just trying to reject that possibility that mean is *equal* to 0; even though we as human thinkers know mean will be greater than 0, this test does *not* reject the possibility that the true mean is negative,

The confidence interval helps get at that reality. The 95% confidence interval of [53.15, 141.47] means that if we were to draw 100 samples from the population and calculate the mean mass for each sample, we would expect 95 of those means to fall between 53.15 and 141.47 kg. Not just above 0, but between those two measures. This is separate from the $p$-value, which is about rejection of the null. There's a necessary mathematical relationship between $p$ and CI, but they are not interpreted in relation to each other.

Keep in mind that despite the fact that we *know* logically this is necessarily true, if we are relying on $t$-tests (or any hypothesis testing) to make the claim, we can only assert extremely strong confidence, not certainty.

We can alternatively specify the mean that the null hypothesis should assume (i.e., a possible mean other than 0 that we are looking to *reject*, not one we want to confirm/support). Let's assume that the `starwars` dataset contains the mass of literally every character in Star Wars. In that case, the *true population mean* mass for Star Wars characters is `r round(mean(sw.desc$mass), 3)` kg. We can specify the null/true mean with the `mu` ($/mu$) argument.
...
This is a case where median might be a better measure measure of center than mean, and a great example of why it's important to always look at descriptive statistics before inferential stats. If we get rid of the extreme outlier and use that as the "true" mean, things might look different.
...
Now randomly sampling from the dataframe will usually *not* be significantly different from that mean. Sometimes it will be though, just because of random variation. Sometimes it will be *extremely* significantly different. Why?
...
While a 1-sample $t$-test compares a sample mean against a static value (like 0 or defined $\mu$), a 2-sample $t$-test compares two sample means against each other. The null hypothesis of a 2-sample $t$-test is that the true means of the group are not different.
...
# (male or female)
# we can use a formula syntax instead of filtering and specifying 
...
Returning to the mass example, where it would be logically absurd for mass to be negative, we could use a 1-tailed test to reject a null that mean mass is *less than or equal to 0.* 

```{r}
#| label: ttest-mass-1tail

# Perform a 1-tailed t-test on the mass variable with a specified mean
# Here the specified mean is 0, but the null hypothesis is that the true mean is
# less than or equal to 0

t.test(sw.desc$mass, alternative = "greater")

```

Again, predictably, we find a strong significant effect. The 1-sided $t$-test shows us it is extraordinarily unlikely ($p$ < .0001) that our alternative hypothesis is incorrect (i.e., true mean mass is *not* greater than 0).
...
The key difference between this example and the mass example is that the psychologist is not comparing 1 group to a possible true mean. She is looking for a directional relationship between means of 2 groups: a 2-sample, 2-tailed test.
...
> **Applied Example:** A psychologist wants to compare the effectiveness of three different educational interventions (parent-guided, teacher-guided, student-guided) on reducing student test anxiety.

One-way ANOVA can be used to test for significant differences in anxiety levels (dependent variable, continuous) across the three interventions (independent variable, factor). 
...
Above we compared male and female characters, but the `starwars` dataset's `sex` variable is not dichotamous. There are several observations where sex is recorded as something else or missing. The above wrangling chunk combines these into an "other" category, creating a variable with 3 levels, perfect for ANOVA comparison.
...
Using Tukey post-hoc adjustment we can see this difference is primarily driven by the difference in height between those in the "male" and "other" category (though even this difference in means is not significant).

### 2-Way ANOVA

> **Applied Example:** A psychologist conducts a study to investigate the effects of both biological sex (male vs. female) and stress level (low vs. high) on performance in a cognitive task.

In this scenario, there are two independent variables: sex (with two levels: male and female) and stress level (with two levels: low and high). The dependent variable is performance in the cognitive task. Two-way ANOVA would be used to assess the main effects of gender and stress level, as well as their interaction effect on performance. The interaction effect indicates whether the effect of one independent variable depends on the level of the other independent variable (e.g., the effect of stress on task performance is greater for males than females, or vice versa).
...
Shockingly, there is no significant difference. Who could have possibly guessed that the powerful combo of hair color and sex together would not predict height?

Well maybe it's about the interaction. Maybe hair color is predictive of height for males only. That sounds perfectly reasonable and I totally have a reason to think that's the case.

I can use the 2-way ANOVA to look for both main effect and this interaction effect by changing the formula syntax from `+` to `*`. This is the same syntax used in other kinds of modeling that can detect interaction effects, like generalized linear models (GLM).

```{r }
#| label: anova-2way-hair-int

# Perform a 2-way ANOVA comparing the dependent variable (height) 
# across sex and hair color

anova_sex3_hair4_int <- aov(height ~ sex3cat * hair4cat, data=sw.hyp)
summary(anova_sex3_hair4_int)

# You can run post-hoc with the interaction, but because there are 
# 66 unique possible comparisons this is not terrible useful 
# TukeyHSD(anova_sex3_hair4_int)

```

As another example, let's look and sex and gender together. It's a weird combo, but you could probably contrive a theoretical reason to ask about an interaction here if you knew more about Star Wars than I do.
...
Notice that although the output for Tukey post-hoc is functionally doing basically the same thing for both 1-way and 2-way ANOVAs (pairwise comparison), in 2-way ANOVA it is only comparing the levels of one factor variable at a time, not the combinations of the two. So in the
...
We've pretty much exhausted the power of Star Wars hair and sex, so let's switch it up to something more likely to show actual effects. We can use the `mtcars` dataset to show a simple example: Does horsepower and/or weight predict a car's fuel consumption?
...
The Chi-Square Test is used to determine whether there is a significant association between categorical variables. It tests for "independence" (the null hypothesis), where there is no association. You can think of it like a kind of conceptual "correlation" among categorical data.
...
Poisson regression is a generalized linear model (GLM) used when your outcome variable is a **count**. It's a common choice when you want to model the expected number of events in some unit of time or space (symptoms per week, errors per task, behaviors per observation period).

> **Applied example:** A psychologist tests whether total word tokens children produced in 1 hour increases over time, as measured at monthly recording sessions.

A Poisson GLM is fit with `glm()` just like logistic regression, but you set `family = poisson`.

We are switching up the data again here to use something with a count outcome. The `warpbreaks` dataset is a built-in dataset that contains the number of warp breaks per loom, where a loom corresponds to a fixed length of yarn. We can use this dataset to model the relationship between the type of yarn and the number of warp breaks using poisson GLM.^[If anyone has an idea for a fish-related dataset with clear count outomes I can use as an example, let me know.]

```{r}
#| label: poisson-warpbreaks
...
Interpreting the results, you see:

1. intercept: the expected number of warp breaks for the reference category (wool A, tension L) is `r round(exp(coef(poisson_model)[["(Intercept)"]]), 3)` breaks per loom
2. woolB: the expected number of warp breaks for wool B is `r round(exp(coef(poisson_model)[["woolB"]]), 3)` times the expected number of breaks for wool A, holding tension constant.
3. tensionM: the expected number of warp breaks for tension M is `r round(exp(coef(poisson_model)[["tensionM"]]), 3)` times the expected number of breaks for tension L, holding wool constant. 
4. tensionH: the expected number of warp breaks for tension H is `r round(exp(coef(poisson_model)[["tensionH"]]), 3)` times the expected number of breaks for tension L, holding wool constant. 

All effects of wool and tension are significant, which means that the expected number of breaks is significantly different for wool B than A, and for tension M and H than L.

Reference category matters! Examine the levels of both factors. The first levels are wool "A" and tension "L" (light). Wool B is compared directly to the reference Wool A. Both tensions "M" (medium) and "H" (heavy) are compared to reference tension L.

We could have done this with a Gaussian linear model, but the Poisson GLM is more appropriate for count data. If we fit the same model without specifying `family=poisson`, we get different results that are less accurate and less meaningful.

```{r}
#| label: not-poisson-warpbreaks


# Fit Poisson regression model

not_poisson_model <- glm(breaks ~ wool + tension, data = warpbreaks)

# Summary of the model

summary(not_poisson_model)
```
...
Generalized linear mixed-effects models (GLMMs) are an extension of generalized linear models (GLMs) that include random effects. Random effects are used when your observations are clustered or **nested** (aka "hierarchical"), meaning they are not independent.

Common "nesting" structures in psychology:

- repeated measures (many observations per participant)
- trials nested within participants (e.g., RT tasks)
- students nested within classrooms (and classrooms within schools)
- therapy clients nested within therapists
...
Conceptually, GLMMs let you combine:

1. a **distribution family** appropriate for the outcome (e.g., binomial, Poisson), and  
2. a **multilevel structure** (random intercepts and sometimes random slopes)

In R, the most common workflow is `lme4::glmer()` (analogous to `glm()`).

#### Random intercept example

A random intercept model assumes each subject has their own baseline level of the outcome.

```{r}
#| label: glmm-sleepstudy-random-intercept

# lme4 has a built-in repeated-measures dataset

sleep <- lme4::sleepstudy

# Mixed model (Gaussian outcome) to illustrate the random-effects idea:
# Reaction time changes over days, and each subject has their own baseline

m_lmm <- lmer(Reaction ~ Days + (1 | Subject), data = sleep)

summary(m_lmm)

```

- `Reaction ~ Days` is the fixed effect, which is the overall average relationship.
- `(1 | Subject)` is the random effect, which lets each subject have a different baseline.

#### Making it "generalized" (non-Gaussian outcomes)

If your outcome is **binary** (yes/no) or **count** data, you use `glmer()` and specify the distribution family, just like with `glm()`.

- Binary outcome: `family = binomial`
- Count outcome: `family = poisson`

```{r}
#| label: glmm-binary-demo

# Toy demo: binary outcome with repeated observations within person

demo_bin <- tibble(
    person = rep(1:40, each = 10),
    stress = rnorm(40 * 10)
    ) |>
    mutate(
        # person-specific baseline propensity (random intercept idea, simulated)
        b0_person = rep(rnorm(40, 0, 0.8), each = 10),
        # generate binary outcome
        y = rbinom(n(), size = 1, prob = plogis(-0.5 + 0.7 * stress + b0_person))
        )

# Logistic mixed model (binary outcome) with random intercept for person

m_glmm_bin <- glmer(y ~ stress + (1 | person), data = demo_bin, family = binomial)
...
If you think the effect of a predictor differs by person (not just the baseline), you can include a random slope:

- `(stress | person)` means baseline *and* stress effect can vary across people.

```{r}

#| label: glmm-random-slope

m_glmm_bin_rs <- glmer(y ~ stress + (stress | person), data = demo_bin, family = binomial)

summary(m_glmm_bin_rs)

```


**Fixed vs. random effects**: What's the difference? Who decides?

The difference between fixed and random effects is more conceptual than statistical. A fixed effect is a predictor variable **that you are specifically interested in testing** (e.g., stress level), while a random effect is a grouping variable that accounts for non-independence in the data (e.g., person).

Using a random-effects structure should match the design of your study and be theoretically motivated. A reasonable default is to start simple (random intercepts), then add complexity if you have a clear reason and the model fits stably.  

In the example above, stress is what we care about, so it's a fixed effect. It's also *part* of a random effect (slope). It's in there twice because we want to test the overall effect of stress across people *and* allow that effect to vary across people. The person variable is only a random effect because we are not interested in testing differences between specific people, but we need to account for the fact that observations within the same person are not independent.

## Slopes and intercepts: models with `1` and `0`: 

In R's model formula syntax, `+ 1` and `+ 0` (or `- 1`) control whether the model includes an intercept term.

The *intercept* is the predicted value of the outcome when all predictors are 0 (and for the reference level of any factors).

- In linear regression: expected **outcome value** when predictors are 0
- In logistic regression: expected **log-odds** when predictors are 0
- In Poisson regression: expected **log expected count** when predictors are 0

This is the default is to include the intercept with `1` or nothing. These are equivalent:

- `y ~ x`
- `y ~ 1 + x`

So you'll mostly see `+ 1` only when someone is being explicit or doing something fancy with formula construction.

Remove the intercept with `+ 0` or `- 1`.
...
**Why do this?** In psychology (and other social sciences), we frequently have meaningless intercepts. The interpretation of a regression's intercept "Y is 10 when X is 0" is nonsensical when the predictor X *literally cannot be 0*, for example in psychology:

-   Age when your research question only concerns adults
-   Hours of sleep when you would exclude participants who literally did not sleep^[They've clearly got other stuff going on.]
-   Iterative predictors like trial number if trials start at 1: the intercept corresponds to "trial 0," which does not exist.
-   Likert scales (when you choose to treat them as continuous), where you have defined min and max and 0 is less than the minimum possible value.

Removing the intercept for models with categorical predictors can also be useful when you want to directly estimate group means rather than differences from a reference level. For example, with `y ~ group`, the coefficient for group B is the difference between group B and the reference group A. With `y ~ group + 0`, the coefficient for group B is the mean of group B itself.

- With intercept: coefficients are usually *differences from a reference level* (for factors) and *slopes around that baseline*.
- Without intercept: coefficients often become *group-specific means* (or group-specific log-means, log-odds, etc.).
